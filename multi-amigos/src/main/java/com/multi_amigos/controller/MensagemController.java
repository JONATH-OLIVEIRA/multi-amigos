...
public class MensagemController {
    ...
    public ResponseEntity<?> criarMensagem(@RequestBody CriarMensagemDTO dto, 
    ...
    // Listar minhas mensagens (mensagens que eu criei)
    @GetMapping("/minhas")
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<List<MensagemDTO>> listarMinhasMensagens(Authentication authentication) {
        ...
        // Aqui você precisa adicionar um método no service para buscar por autor
        // Por enquanto, vamos retornar todas
        List<MensagemDTO> mensagens = mensagemService.listarTodasMensagens();
        return ResponseEntity.ok(mensagens);
    }
    ...
    @PutMapping("/{id}")
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<MensagemDTO> atualizarMensagem(@PathVariable Long id, 
