...
public interface UsuarioRepository extends JpaRepository<Usuario, Long> {
	...
	@Query(value = "WITH RECURSIVE descendentes AS (" + "  SELECT id, nome, email, usuario_pai_id, ativo, perfil "
			+ "  FROM usuarios WHERE id = ?1 " + "  UNION ALL "
			+ "  SELECT u.id, u.nome, u.email, u.usuario_pai_id, u.ativo, u.perfil " + "  FROM usuarios u "
	...
	@Query(value = "WITH RECURSIVE ancestrais AS (" + "  SELECT id, usuario_pai_id, 1 as nivel "
			+ "  FROM usuarios WHERE id = ?1 " + "  UNION ALL " + "  SELECT u.id, u.usuario_pai_id, a.nivel + 1 "
			+ "  FROM usuarios u " + "  INNER JOIN ancestrais a ON u.id = a.usuario_pai_id" + ") "
	...
	@Query(value = "SELECT " + "  COUNT(*) as total_usuarios, "
			...
			+ "  MAX(profundidade) as maior_profundidade " + "FROM (" + "  SELECT *, "
			+ "  (WITH RECURSIVE profundidade AS (" + "    SELECT id, usuario_pai_id, 1 as nivel "
			+ "    FROM usuarios WHERE id = u.id " + "    UNION ALL "
			+ "    SELECT u2.id, u2.usuario_pai_id, p.nivel + 1 " + "    FROM usuarios u2 "
			+ "    INNER JOIN profundidade p ON u2.id = p.usuario_pai_id" + "  ) "
			+ "  SELECT MAX(nivel) FROM profundidade) as profundidade " + "  FROM usuarios u"
	...
	@Query(value = "SELECT DATE(data_criacao) as dia, COUNT(*) as quantidade " + "FROM usuarios "
			+ "WHERE data_criacao >= :dataInicio " + "GROUP BY DATE(data_criacao) "
	...
	@Query(value = "SELECT perfil, COUNT(*) as quantidade " + "FROM usuarios " + "WHERE ativo = true "
	...
	@Query(value = "WITH RECURSIVE ancestrais AS (" + "  SELECT id, usuario_pai_id " + "  FROM usuarios WHERE id = ?1 "
			+ "  UNION ALL " + "  SELECT u.id, u.usuario_pai_id " + "  FROM usuarios u "
	...
	@Query("SELECT u, COUNT(f) as totalFilhos " + "FROM Usuario u LEFT JOIN u.filhos f "
			+ "WHERE f.ativo = true OR f IS NULL " + "GROUP BY u.id")
